...
<nav class="navbar" aria-label="main navigation">
  <div class="navbar-brand">
    <Link class="navbar-item" to="/">
      <img src="logo.png" width="112" height="28" alt="Pic" />
    </Link>
  ...
  <div id="navbarMain" class="navbar-menu {showHamMenu ? 'is-active' : ''}">
    <div class="navbar-start">
      <Link
        to="/"
        class="navbar-item {url == '/' ? 'has-text-weight-semibold' : ''}"
        >Home</Link
      >
      ...
      <Link
        to="/about"
        class="navbar-item {url == '/about' ? 'has-text-weight-semibold' : ''}"
        >About</Link
      >

      <div
        use:clickOutside
        on:click_outside={clickOutsideMoreMenu}
        on:click={() => (showMoreMenu = !showMoreMenu)}
        class="navbar-item has-dropdown {showMoreMenu ? 'is-active' : ''}"
      >
        <span class="navbar-link">More</span>

        <div class="navbar-dropdown">
          <a href="https://github.com/FinnKr/pixlr" class="navbar-item"
            >GitHub Repository</a
          >
          <a href="https://github.com/FinnKr/pixlr/issues" class="navbar-item"
            >Report an issue</a
          >
        </div>
      </div>
      ...
      <div class="navbar-item">
        <div class="buttons">
          {#if logged_in}
            ...
            <button
              on:click={logout}
              class="button is-outlined is-success has-icons has-icons-right"
            >
              <p>Log Out</p>
            ...
            <Link
              to="/profile"
              class="button is-rounded has-text-weight-semibold  {url ==
              '/profile'
                ? 'is-active'
                : ''}">{account.name.substring(0, 1)}</Link
            >
          {:else}
            ...
            <Link
              to="/login"
              class="button is-light  {url == '/login'
